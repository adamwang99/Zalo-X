 ...
 *
 * Patches the Electron dev binary with Zalo-X's icon and version metadata.
...
async function main() {
  ...
  try {
    ...
    await rcedit(electronExePath, {
      icon: iconPath,
      'version-string': {
        ProductName:     pkg.build?.productName || 'Zalo-X',
        FileDescription: pkg.build?.productName || 'Zalo-X',
        CompanyName:     'Zalo-X',
      },
    ...
    console.log('[patch-icon] ✅ electron.exe patched with Zalo-X icon');
  } catch (err) {
